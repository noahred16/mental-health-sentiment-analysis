...
class TestTFIDFMethod:
    ...
    def test_tfidf_checkpoint_loading_exists(self):
        ...
        if os.path.exists(checkpoint_path):
            ...
            pytest.skip("TF-IDF checkpoint not found - create one by training the model first")
...
class TestLSTMMethod:
    ...
    def test_lstm_predict_sentiment(self):
        ...
        if os.path.exists(checkpoint_path):
            ...
            assert isinstance(probabilities, (list, tuple, np.ndarray))  # Accept numpy arrays too
...
class TestRoBERTaMethod:
    ...
    def test_roberta_predict_sentiment(self):
        ...
        if os.path.exists(checkpoint_path):
            ...
            # Test prediction
            test_text = "I feel anxious"
            prediction, probabilities = roberta.predict_sentiment(test_text, model, tokenizer, label_encoder)
    ...
    def test_all_demo_functions_consistent_format(self):
        ...
        for method_name, demo_func in methods_to_test:
            ...
            if results is not None:  # Skip if model failed to load
                ...
                assert expected_keys.issubset(result.keys()), f"{method_name} missing required keys"
                ...
                probs = [prob for _, prob in result["probabilities"]]
                assert probs == sorted(probs, reverse=True), f"{method_name} probabilities not sorted"
